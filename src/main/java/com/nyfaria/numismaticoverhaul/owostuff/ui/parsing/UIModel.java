...
//            //Owo.LOGGER.warn("Could not load UI model from file {}", path, error);
     ...
     *                                   root component to create, pass {@link ModComponent}
     */
     ...
     *                         the type of component to parse, pass {@link ModComponent}
     * @param componentElement The XML element represented the
     *                         component to parse.
     * @return The parsed component
     */
    @SuppressWarnings("unchecked")
    public <T extends ModComponent> T parseComponent(Class<T> expectedClass, Element componentElement) {
     ...
     *                          have. Should this be violated, an exception is
     *                          thrown. If there are no specific expectations about
     *                          the type of component to create, pass {@link ModComponent}
     ...
     */
    @SuppressWarnings("unchecked")
    public <T extends ModComponent> T expandTemplate(Class<T> expectedClass, String name, Function<String, String> parameterSupplier, Function<String, Element> childSupplier) {
        ...
        final var component = this.parseComponent(ModComponent.class, UIParsing.<Element>allChildrenOfType(template, Node.ELEMENT_NODE).get(0));
        if (!expectedClass.isAssignableFrom(component.getClass())) {
            throw new IncompatibleUIModelException(
     ...
     * @return The expanded template parsed into a component
     */
    public <T extends ModComponent> T expandTemplate(Class<T> expectedClass, String name, Map<String, String> parameters) {
